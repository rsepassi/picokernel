...
static inline void platform_mem_validate_post_init(platform_t *platform, void *fdt) { (void)platform; (void)fdt; }
static inline void platform_mem_print_layout(void) {}
static inline void platform_mem_dump_translation(uintptr_t vaddr) { (void)vaddr; }
...
static inline void platform_mem_set_guard(void* addr, uint32_t size) { (void)addr; (void)size; }
static inline bool platform_mem_check_guard(void* addr, uint32_t size) { (void)addr; (void)size; return true; }
static inline void kmem_dump(const void* addr, uint32_t len) { (void)addr; (void)len; }
static inline void kmem_dump_range(const char* label, const void* start, const void* end) { (void)label; (void)start; (void)end; }
static inline bool kmem_validate_pattern(const void* addr, uint32_t len, uint8_t pattern) { (void)addr; (void)len; (void)pattern; return true; }
static inline bool kmem_ranges_overlap(uintptr_t a_start, uint32_t a_size, uintptr_t b_start, uint32_t b_size) { (void)a_start; (void)a_size; (void)b_start; (void)b_size; return false; }
static inline uint32_t kmem_crc32(const void* data, uint32_t len) { (void)data; (void)len; return 0; }
static inline uint32_t kmem_checksum_section(const void* start, const void* end) { (void)start; (void)end; return 0; }
#endif

...
void platform_mem_validate_critical(void) {
    ...
    if (kmem_ranges_overlap(DTB_BASE, DTB_SIZE, text_start, kernel_end - text_start)) {
    ...
    if (expected_text_crc == 0 && expected_rodata_crc == 0) {
        printk("[MEM] WARNING: Expected checksums not set (build may not have run checksum script)\n");
    } else {

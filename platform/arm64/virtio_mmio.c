...
static void virtio_rng_irq_handler(void* context) {
    ...
    uint32_t isr_status = mmio_read32(rng->mmio_base, VIRTIO_MMIO_INTERRUPT_STATUS);
...
void virtio_rng_mmio_setup(platform_t* platform, uint64_t mmio_base, uint64_t mmio_size, uint32_t irq_num) {
    ...
    uint32_t queue_num_max = mmio_read32(rng->mmio_base, VIRTIO_MMIO_QUEUE_NUM_MAX);
    ...
    if (version == 1) {
        ...
        mmio_write32(rng->mmio_base, VIRTIO_MMIO_QUEUE_DESC_LOW, (uint32_t)desc_addr);
        mmio_write32(rng->mmio_base, VIRTIO_MMIO_QUEUE_DESC_HIGH, (uint32_t)(desc_addr >> 32));

        uint64_t avail_addr = (uint64_t)rng->vq.avail;
        mmio_write32(rng->mmio_base, VIRTIO_MMIO_QUEUE_DRIVER_LOW, (uint32_t)avail_addr);
        mmio_write32(rng->mmio_base, VIRTIO_MMIO_QUEUE_DRIVER_HIGH, (uint32_t)(avail_addr >> 32));

        uint64_t used_addr = (uint64_t)rng->vq.used;
        mmio_write32(rng->mmio_base, VIRTIO_MMIO_QUEUE_DEVICE_LOW, (uint32_t)used_addr);
        mmio_write32(rng->mmio_base, VIRTIO_MMIO_QUEUE_DEVICE_HIGH, (uint32_t)(used_addr >> 32));
    ...
    // CRITICAL: Clean entire virtqueue memory to ensure device sees initialized state
    // This includes descriptor table, available ring, and used ring
...
void kplatform_tick(platform_t* platform, kernel_t* k) {
    ...
    while (virtqueue_has_used(&rng->vq)) {
        ...
        if (req != NULL) {
            // CRITICAL: Invalidate buffer cache - device wrote random data to this buffer

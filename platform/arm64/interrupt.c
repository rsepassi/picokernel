...
#define GICD_ICENABLER_OFF 0x180
#define GICD_IPRIORITYR_OFF 0x400
#define GICD_ITARGETSR_OFF 0x800
#define GICD_ICFGR_OFF 0xC00
...
static const char *exception_names[] = {"Synchronous EL1t",
                                        "IRQ EL1t",
                                        "FIQ EL1t",
                                        "SError EL1t",
                                        "Synchronous EL1h",
...
void exception_handler(uint64_t type, uint64_t esr, uint64_t elr,
                       uint64_t far) {
  ...
  if (type == 5) { // IRQ from current EL with SP_ELx (most common for kernel)
    ...
    mmio_write32(g_current_platform->gic_cpu_base + GICC_EOIR_OFF, irq);
  } else {
...
void irq_dump_config(platform_t *platform, uint32_t irq_num) {
  ...
  // Check if enabled
  uint32_t enable_reg = gicd_base + GICD_ISENABLER_OFF + (irq_num / 32) * 4;
  ...
  uint32_t priority_reg = gicd_base + GICD_IPRIORITYR_OFF + (irq_num / 4) * 4;
  ...
  // Check target CPU
  uint32_t target_reg = gicd_base + GICD_ITARGETSR_OFF + (irq_num / 4) * 4;
  ...
  // Check trigger type
  uint32_t cfg_reg = gicd_base + GICD_ICFGR_OFF + (irq_num / 16) * 4;
  ...
  uint32_t gicc_ctlr = mmio_read32(gicc_base + GICC_CTLR_OFF);
  uint32_t gicc_pmr = mmio_read32(gicc_base + GICC_PMR_OFF);
  printk("  GICC_CTLR: 0x");
  ...
  uint32_t gicd_ctlr = mmio_read32(gicd_base + GICD_CTLR_OFF);
  printk("  GICD_CTLR: 0x");

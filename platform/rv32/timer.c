...
static const uint8_t* fdt_find_property(void* fdt, const char* node_path,
                                         const char* prop_name, uint32_t* out_len)
{
    ...
    while (1) {
        ...
        if (token == FDT_BEGIN_NODE) {
            ...
            if (name[0] == 'c' && name[1] == 'p' && name[2] == 'u' && name[3] == 's') {
                in_cpus_node = 1;
            }
...
void timer_init(void* fdt)
{
    ...
    if (fdt) {
        uint32_t len;
        const uint8_t* prop = fdt_find_property(fdt, "/cpus", "timebase-frequency", &len);

        if (prop && len == 4) {
            // Read as big-endian 32-bit value
            g_timer_freq = (prop[0] << 24) | (prop[1] << 16) | (prop[2] << 8) | prop[3];
            ...
            g_timer_freq = (prop[4] << 24) | (prop[5] << 16) | (prop[6] << 8) | prop[7];
            printk("Timer frequency from DT: ");
            printk_dec(g_timer_freq);
            printk(" Hz\n");
        } else {
...
void timer_set_oneshot_ms(uint32_t milliseconds, timer_callback_t callback)
{
    ...
    // For typical values (10MHz * 1000ms = 10M ticks), this won't overflow 64 bits

...
static void* fw_cfg_find_rsdp(void)
{
    ...
    if (tables_selector != 0 && tables_file_size <= sizeof(tables_buf)) {
        ...
        while (ptr + sizeof(struct acpi_table_header) < end && table_count < 32) {
            ...
            if (header->length == 0 || header->length < sizeof(struct acpi_table_header) ||
        ...
        rsdt_build->header.length = sizeof(struct acpi_table_header) + (table_count * 4);
...
struct acpi_table_header* acpi_find_table(const char* signature)
{
    ...
    if (g_rsdp->rsdt_address != 0) {
        ...
        volatile uint32_t entries = (rsdt->header.length - sizeof(struct acpi_table_header)) / 4;
...
void acpi_init(void)
{
    ...
    printk_hex32((uint32_t)(uintptr_t)g_rsdp);
    printk(", Revision ");

...
static uint32_t ioapic_read_reg(platform_t *platform, uint8_t reg) {
...
static uint64_t ioapic_read_redtbl(platform_t *platform, uint8_t entry) {
  uint32_t low = ioapic_read_reg(platform, IOAPIC_REDTBL_BASE + entry * 2);
  uint32_t high = ioapic_read_reg(platform, IOAPIC_REDTBL_BASE + entry * 2 + 1);
...
static void ioapic_write_redtbl(platform_t *platform, uint8_t entry,
  ...
  ioapic_write_reg(platform, IOAPIC_REDTBL_BASE + entry * 2, low);
  ioapic_write_reg(platform, IOAPIC_REDTBL_BASE + entry * 2 + 1, high);
}
...
void ioapic_init(platform_t *platform) {
  ...
  uint32_t version = ioapic_read_reg(platform, IOAPIC_REG_VERSION);
  ...
  for (uint8_t i = 0; i < platform->ioapic.max_entries; i++) {
    ...
    ioapic_write_redtbl(platform, i, entry);
  }
...
void ioapic_route_irq(platform_t *platform, uint8_t irq, uint8_t vector,
  ...
  ioapic_write_redtbl(platform, irq, entry);
}
...
void ioapic_mask_irq(platform_t *platform, uint8_t irq) {
  ...
  uint64_t entry = ioapic_read_redtbl(platform, irq);
  entry |= IOAPIC_MASK;
  ioapic_write_redtbl(platform, irq, entry);
}

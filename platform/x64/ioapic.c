...
static uint32_t ioapic_read_reg(uint8_t reg) {
    volatile uint32_t* ioregsel = (volatile uint32_t*)(uintptr_t)g_ioapic.base_addr;
    volatile uint32_t* iowin = (volatile uint32_t*)(uintptr_t)(g_ioapic.base_addr + IOWIN_OFFSET);
...
static int find_ioapic_in_madt(void) {
    ...
    while (ptr < end) {
        ...
        if (entry->type == ACPI_MADT_TYPE_IO_APIC) {
            struct acpi_madt_io_apic* ioapic_entry = (struct acpi_madt_io_apic*)entry;

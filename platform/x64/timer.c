...
static inline uint32_t lapic_read(platform_t *platform, uint32_t reg) {
  return *(volatile uint32_t *)(platform->lapic_base + reg);
}
...
static void calibrate_timer(platform_t *platform) {
  // Set divisor to 16
  lapic_write(platform, LAPIC_TIMER_DIV, 0x3);
  ...
  lapic_write(platform, LAPIC_TIMER_INIT, 0xFFFFFFFF);
  ...
  // Stop LAPIC timer
  lapic_write(platform, LAPIC_LVT_TIMER, 32 | 0x10000);
  lapic_write(platform, LAPIC_TIMER_INIT, 0);
  ...
  if (tsc_elapsed > 0) {
    platform->tsc_per_ms = tsc_elapsed / 10;
    printk("TSC calibrated: ");
    printk_dec((uint32_t)(platform->tsc_per_ms / 1000));
    printk(" MHz\n");
  } else {
    // Default to 1GHz if calibration fails
    platform->tsc_per_ms = 1000000;
...
void timer_init(platform_t *platform) {
  ...
  // Capture start time
  platform->tsc_start = rdtsc();
}

// Get current time in milliseconds
uint64_t timer_get_current_time_ms(platform_t *platform) {
  uint64_t tsc_now = rdtsc();
  uint64_t tsc_elapsed = tsc_now - platform->tsc_start;

  if (platform->tsc_per_ms == 0) {
    return 0;
  }

  return tsc_elapsed / platform->tsc_per_ms;
}
...
void timer_set_oneshot_ms(platform_t *platform, uint32_t milliseconds,
  ...
  // - Not masked (bit 16 = 0)
  lapic_write(platform, LAPIC_LVT_TIMER, 32 | TIMER_MODE_ONESHOT);
...
void timer_cancel(platform_t *platform) {
  lapic_write(platform, LAPIC_LVT_TIMER,
              32 | 0x10000); // Vector 32, masked (bit 16)
}

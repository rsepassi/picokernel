...
static int virtio_find_capabilities(virtio_rng_t* rng) {
    uint8_t cap_offset = pci_config_read8(rng->pci_bus, rng->pci_slot, rng->pci_func, PCI_REG_CAPABILITIES);
    ...
    while (cap_offset != 0) {
        uint8_t cap_id = pci_config_read8(rng->pci_bus, rng->pci_slot, rng->pci_func, cap_offset);

        if (cap_id == 0x09) {  // Vendor-specific capability
            uint8_t cfg_type = pci_config_read8(rng->pci_bus, rng->pci_slot, rng->pci_func, cap_offset + 3);
            uint8_t bar = pci_config_read8(rng->pci_bus, rng->pci_slot, rng->pci_func, cap_offset + 4);
            uint32_t offset = pci_config_read32(rng->pci_bus, rng->pci_slot, rng->pci_func, cap_offset + 8);

            if (cfg_type == VIRTIO_PCI_CAP_COMMON_CFG) {
                rng->common_cfg_bar = pci_read_bar(rng->pci_bus, rng->pci_slot, rng->pci_func, bar);
                rng->common_cfg_offset = offset;
                rng->common_cfg = (volatile virtio_pci_common_cfg_t*)(rng->common_cfg_bar + offset);
                found_common = 1;
            } else if (cfg_type == VIRTIO_PCI_CAP_NOTIFY_CFG) {
                rng->notify_bar = pci_read_bar(rng->pci_bus, rng->pci_slot, rng->pci_func, bar);
                rng->notify_offset = offset;
                rng->notify_off_multiplier = pci_config_read32(rng->pci_bus, rng->pci_slot, rng->pci_func, cap_offset + 16);
                rng->notify_base = (volatile uint32_t*)(rng->notify_bar + offset);
                found_notify = 1;
            } else if (cfg_type == VIRTIO_PCI_CAP_ISR_CFG) {
                rng->isr_bar = pci_read_bar(rng->pci_bus, rng->pci_slot, rng->pci_func, bar);
        ...
        cap_offset = pci_config_read8(rng->pci_bus, rng->pci_slot, rng->pci_func, cap_offset + 1);
    }
...
void virtio_rng_setup(platform_t* platform, uint8_t bus, uint8_t slot, uint8_t func) {
    ...
    command &= ~PCI_CMD_INT_DISABLE;  // Clear interrupt disable bit (enable interrupts)
    ...
    // Acknowledge device
    mmio_write8((volatile uint8_t*)&rng->common_cfg->device_status, VIRTIO_STATUS_ACKNOWLEDGE);
    ...
    // Set queue addresses (physical addresses - in QEMU they're the same as virtual)

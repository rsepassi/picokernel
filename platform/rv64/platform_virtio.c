// RISC-V 64-bit VirtIO Platform Integration
...
// No global device state - all moved to platform_t
...
static void virtio_rng_setup(platform_t *platform, uint8_t bus, uint8_t slot,
                             uint8_t func) {
  // Assign BAR addresses (bare-metal RISC-V needs to do this manually)
  // Start address for PCI MMIO region (from QEMU virt DT: 0x40000000)
  uint64_t bar_addr = 0x40000000;
  ...
  // RISC-V PLIC external interrupts start at 1
...
#define VIRTIO_MMIO_IRQ_BASE 1       // PLIC external IRQs start at 1

// RISC-V 64-bit PCI Configuration Space Access Implementation
...
// ECAM base address for QEMU RISC-V virt machine
// Base address: 0x30000000, size: 256MB (supports 256 buses)
...
uint64_t platform_pci_read_bar(platform_t *platform, uint8_t bus, uint8_t slot,
  ...
  if (bar_low & 0x1) {
    // I/O space - return 0 as we don't support I/O BARs on RISC-V
    return 0;
  }

...
in `kmain_usermain()` (src/user.c). The default user code requests random
...
(In the repo, you'll see Monocypher, which provides ChaCha20 for the kernel's
CSPRNG, seeded by hardware randomness, but it's not core to the kernel).

The kernel.elf binaries weigh in at:
